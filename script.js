const surahs = [
    ...
    { id: 2, name: "البقرة", juz: 1 },

    { id: 3, name: "آل عمران", juz: 3 },

    { id: 4, name: "النساء", juz: 4 },
...
surahs.forEach(surah => {
    ...
    label.innerText = `${surah.name} (الجزء ${surah.juz})`;
...
function calculateProgress() {
    ...
    const checkedSurahs = document.querySelectorAll("input[type='checkbox']:checked").length;
    ...
    document.querySelectorAll("input[type='checkbox']:checked").forEach(checkbox => {
        ...
        const surah = surahs.find(s => `surah-${s.id}` === checkbox.id);

        if (surah) completedJuz.add(surah.juz);

    });



    const completionRate = ((checkedSurahs / totalSurahs) * 100).toFixed(2);
